...
def run_fio(iodepth, numjobs, test_type, size):
    cmd = [
        "sudo", "LD_PRELOAD=/home/arian/spdk/build/fio/spdk_nvme",
        "fio", "--name=test", "--filename=trtype=PCIe traddr=0001.10.00.0 ns=1",
        "--direct=1", "--rw={}".format(test_type), "--bs=4k", "--iodepth={}".format(iodepth),
        "--numjobs={}".format(numjobs), "--size={}".format(size), "--ioengine=spdk",
        "--group_reporting", "--norandommap=1", "--thread=1", "--output-format=json"
    ]
...
def main():
    ...
    with open("spdk_results.json", "w") as f:
